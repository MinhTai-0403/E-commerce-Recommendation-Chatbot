...
const { handleAuthRequest, verifyJwt } = require("./auth-service");
const { extractCellphonesDetails } = require("./cellphones-detail-extractor");
...
async function findProductDetailByIdentifier(productDetails, identifier, product) {
  ...
  for (const lookup of exactLookups) {
    ...
    const detail = await productDetails.findOne(lookup, {
      sort: { updatedAt: -1, scrapedAt: -1 },
    });
    if (detail) return detail;
  }

  return productDetails.findOne(buildProductDetailLookup(identifier, product), {
    sort: { updatedAt: -1, scrapedAt: -1 },
  });
}
...
function buildCategoryCondition(category = "") {
  ...
  return {
    $or: regexes.map((regex) => ({ categories: regex })),
  };
}
...
async function persistProductDetailManifest({ productDetails, products, detail, product }) {
...
async function handleListProducts(req, res) {
  const { products, productDetails } = await getDb();
  ...
  const listDocs = !includeRaw && includeDetails
    ? await mergeProductListWithDetailSummaries(productDetails, docs)
    : docs;
  ...
  sendJson(res, 200, {
    ...
    data: includeRaw ? docs : listDocs.map(normalizeProduct),
  });
}

async function handleGetProduct(_req, res, identifier) {
  const { products } = await getDb();
  const doc = await findProductByIdentifier(products, identifier);
...
async function handleGetProductDetails(req, res, identifier) {
  ...
  const url = new URL(req.url, `http://${req.headers.host}`);
  const includeRaw = url.searchParams.get("raw") === "true";
  const product = await findProductByIdentifier(products, identifier);
  ...
  if (!detail) {
    ...
    try {
      ...
      await persistProductDetailManifest({ productDetails, products, detail, product });
...
async function handleListProductReviews(req, res, identifier) {
  const { products, productReviews } = await getDb();
  const product = await findProductByIdentifier(products, identifier);
...
async function handleListProductQuestions(req, res, identifier) {
  const { products, productQuestions } = await getDb();
  const product = await findProductByIdentifier(products, identifier);
...
async function handleUpdateProduct(req, res, identifier) {
  ...
  const existing = await findProductByIdentifier(products, identifier);
...
async function handleDeleteProduct(_req, res, identifier) {
  ...
  const result = await products.findOneAndDelete({ _id: existing._id });
