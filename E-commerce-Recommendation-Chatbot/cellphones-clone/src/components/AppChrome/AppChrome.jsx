...
const SharedHeroSection = lazy(() => import('../HeroSection/HeroSection'));
...
const HOME_MEGA_CATEGORY_BY_SECTION = {
  'phone-tablet': 'phone',
  laptop: 'laptop',
  audio: 'audio',
  watch: 'watch',
...
const MEGA_SIDEBAR_LABEL_CATEGORIES = {
  Tablet: 'Máy tính bảng',
  'Đồng hồ': 'Đồng hồ thông minh',
};

function getMegaSidebarLabelHref(item, label) {
  if (item.id === 'audio') {
    return getAudioCategoryPath(label) || AUDIO_CATEGORY_PATHS.root;
  }

  if (!item.label.includes(',')) {
    return item.href || buildCategoryPath(item.category || item.label, {
      keyword: item.label,
      title: item.label,
    });
  }

  const category = MEGA_SIDEBAR_LABEL_CATEGORIES[label] || label;
...
function buildMegaHref(label, category = 'Điện thoại', extra = {}) {
  const canonicalAudioPath = getAudioCategoryPath(label);
      ...
      <section className="header-mega-column generic-column" key={group.title}>
        ...
        <div className={group.kind === 'logo' ? 'header-mega-brand-grid' : 'header-mega-generic-grid'}>
          {group.items.map((item) => {
            ...
            const href = section.href || buildMegaHref(label, category || label, {
              q: item.q || label,
              brand: item.brand,
...
export function HeaderPopups({
  ...
  const homeMegaCategory = HOME_MEGA_CATEGORY_BY_SECTION[activeMegaSection.id];
        ...
        <>
          ...
          <nav className="header-home-category-reuse" aria-label="Danh mục sản phẩm">
            <div className="hero-inner header-home-category-inner">
              <div className="hero-sidebar header-home-sidebar-reuse">
                <ul className="category-menu">
                  {megaMenuSections.map((item) => {
                    const labels = item.label.split(',').map((label) => label.trim());
                              ...
                              {labels.map((label, index) => (
                                <span key={label}>
                                  ...
                                  <a
                                    ...
                                    href={getMegaSidebarLabelHref(item, label)}
              ...
              <div className={`header-home-mega-panel-reuse ${homeMegaCategory ? 'header-home-shared-mega' : 'mega-menu-panel'}`}>
                {homeMegaCategory ? (
                  <Suspense fallback={null}>
                    <SharedHeroSection menuOnly activeMenuCategory={homeMegaCategory} />
                  </Suspense>
