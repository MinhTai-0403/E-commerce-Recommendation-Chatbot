...
const getBrandFilter = (brand = "") => {
  ...
  if (/iphone|ipad|airpods|apple watch|macbook|apple/i.test(name))
    return "apple";
...
const getSidebarLabelPath = (cat = {}, slug = "", label = "") => {
  ...
  const canonicalAudioPath =
    slug === "audio" ? getAudioCategoryPath(cleanLabel) : "";

  if (canonicalAudioPath) return canonicalAudioPath;

  if (!cat.name?.includes(",")) {
    return getSidebarCategoryPath(cat, slug);
  }
...
const getPhoneBrandPath = (brandName = "") => {
  ...
  if (isIphone) return "/mobile/apple.html";
...
const TABLET_BRAND_ROUTES = {
  ipad: "/tablet/ipad.html",
...
const getTabletBrandPath = (brandName = "") => {
  ...
  const canonicalPath = TABLET_BRAND_ROUTES[brandKey];
...
const getPhonePricePath = (label = "") => {
  ...
  if (price.includes("Dưới 2"))
    return getPhoneTopicPath(title, { ...options, priceMax: 2_000_000 });
  if (price.includes("2 - 4"))
    return getPhoneTopicPath(title, {
      ...options,
      priceMin: 2_000_000,
      priceMax: 4_000_000,
    });
  if (price.includes("4 - 7"))
    return getPhoneTopicPath(title, {
      ...options,
      priceMin: 4_000_000,
      priceMax: 7_000_000,
    });
  if (price.includes("7 - 13"))
    return getPhoneTopicPath(title, {
      ...options,
      priceMin: 7_000_000,
      priceMax: 13_000_000,
    });
  if (price.includes("13 - 20"))
    return getPhoneTopicPath(title, {
      ...options,
      priceMin: 13_000_000,
      priceMax: 20_000_000,
    });
  if (price.includes("Trên 20"))
    return getPhoneTopicPath(title, { ...options, priceMin: 20_000_000 });
...
export default function HeroSection({
  ...
  const [internalHoveredCategory, setHoveredCategory] = useState(null);
  const hoveredCategory = activeMenuCategory ?? internalHoveredCategory;
  const displayName =
  ...
  return (
    <section className={`hero-section section-gap ${menuOnly ? "hero-section-menu-only" : ""}`}>
      {!menuOnly && hoveredCategory !== null && (
              ...
              <div className="mega-column">
                <div className="mega-section">
                  ...
                  <div className="mega-brand-logos-grid">
                    ...
                    <a
                      href={getPhoneTopicPath("Điện thoại phổ thông", {
                        q: "Điện thoại phổ thông",
                      })}
                ...
                <div className="mega-section">
                  ...
                  <div className="mega-brand-logos-grid">
                    ...
                    {tabletQuickLinks.map((item) => (
                      <a
                        key={item.name}
                        href={item.href || getCategoryTopicPath(item.category, item.name, {
                          q: item.query || item.name,
                          title: item.title || item.name,
                        })}
                        className={`mega-brand-logo-card-item ${item.wide ? "wide-text-pill" : ""}`}
                      >
                        <span className="fallback-brand-text">{item.name}</span>
                      </a>
                    ))}
                  </div>
                </div>
                <div className="mega-section" style={{ marginTop: "18px" }}>
                  <div className="mega-section-title">Máy tính bảng HOT ⚡</div>
                  <div className="mega-grid-pills-flexible">
                    {hotTabletModels.map((tablet, i) => (
                      <a
                        key={i}
                        href={getCategoryTopicPath("Máy tính bảng", tablet.name, {
                          q: tablet.name,
                          title: tablet.name,
                        })}
                        className="mega-pill-item relative-pill"
                        style={{ gridColumn: `span ${tablet.span}` }}
                      >
                        <span>{tablet.name}</span>
                        {tablet.badge && (
                          <span
                            className={`pill-badge-tag ${tablet.badge === "Mới" ? "bg-blue" : "bg-red"}`}
                          >
                            {tablet.badge}
                          </span>
                        )}
          ...
          {hoveredCategory === "laptop" && (
            ...
            >
                ...
                <div className="mega-section" style={{ marginTop: "20px" }}>
                  ...
                  <div className="mega-grid-pills-flexible">
                    {[
                      ...
                      <a
                        key={i}
                        href={getCategoryTopicPath("Laptop", `Laptop ${p}`, {
                          q: p,
                        })}
                        className="mega-pill-item"
                ...
                <div className="mega-section">
                  ...
                  <div className="mega-laptop-needs-grid">
                    {laptopNeedsData.map((item, i) => (
                      <a
                        key={i}
                        href={laptopNeedRoutes[item.name] || getCategoryTopicPath("Laptop", item.name)}
                    ...
                    <a
                      href={getCategoryTopicPath("Laptop", "Mac CTO", {
                        brand: "apple",
                      })}
              ...
              <div className="mega-column">
                ...
                <div className="mega-section" style={{ marginTop: "20px" }}>
                  <div className="mega-section-title">Kích thước màn hình</div>
                  <div className="mega-grid-pills-flexible">
                    {laptopScreenSizes.map((screen, i) => (
                      <a
                        key={i}
                        href={getCategoryTopicPath("Laptop", screen.name, {
                          q: screen.name,
                          title: screen.name,
                        })}
                        className="mega-pill-item"
                        style={{ gridColumn: `span ${screen.span}` }}
                      >
                        {screen.name}
                      </a>
                    ))}
                  ...
                  <div className="mega-laptop-needs-grid">
                    {audioTypesSquare.map((item, i) => (
                      <a
                        key={i}
                        href={
                          getAudioCategoryPath(item.name) ||
                          getCategoryTopicPath("Tai nghe", item.name)
                        }
                        className="mega-laptop-need-card"
                      >
                        <img
                  ...
                  <div className="mega-laptop-needs-grid">
                    {micTypesSquare.map((item, i) => (
                      <a
                        ...
                        href={
                          getAudioCategoryPath(item.name) ||
                          getCategoryTopicPath("Âm thanh", item.name)
                        }
                        className="mega-laptop-need-card"
                      >
                        <img
                ...
                <div className="mega-section">
                  ...
                  <div className="mega-brand-logos-grid">
                    {MONITOR_PC_BRANDS.map((brand, i) => (
                      <a
                        key={i}
                        href={getCategoryBrandPath("Màn hình", brand, "", {
                          segment: "monitor",
                        })}
                        className="mega-brand-logo-card-item"
                      >
                ...
                <div className="mega-section" style={{ marginTop: "20px" }}>
                  ...
                  <div className="mega-laptop-needs-grid">
                    {monitorNeedsRows.map((item, i) => (
                      <a
                        key={i}
                        href={getCategoryTopicPath("Màn hình", item.name, {
                          segment: "monitor",
                        })}
                        className="mega-laptop-need-card grid-wide-row"
                      >
                    ...
                    <a
                      href={buildCategoryPath("Màn hình", {
                        segment: "monitor",
                      })}
                      className="mega-pill-item full-width-row-pill"
                      style={{ gridColumn: "span 2", marginTop: "4px" }}
              ...
              <div className="mega-column">
                <div className="mega-section">
                  <div className="mega-section-header-row">
                    ...
                    <a
                      href={buildCategoryPath("Gaming Gear")}
                      className="mega-inline-text-link"
                    >
                      Xem tất cả
                    </a>
                  </div>
              ...
              <div className="mega-column">
                ...
                <div className="mega-section" style={{ marginTop: "16px" }}>
                  ...
                  <ApplianceBrandGrid
                    ...
                    viewAllLabel="Xem tất cả máy sấy quần áo"
                    viewAllSpan={2}
                  />
                </div>
                <div className="mega-section" style={{ marginTop: "16px" }}>
                  <div className="mega-section-title">
                    Chọn hãng máy rửa chén bát
                  </div>
                  <ApplianceBrandGrid
                    category="Máy rửa chén bát"
                    brands={["Bosch"]}
                    viewAllLabel="Xem tất cả máy rửa chén bát"
                    viewAllSpan={2}
                  />
                </div>
              </div>
          ...
          <div className="hero-slider-wrapper">
              ...
              >
                {heroSlides.map((slide, index) => (
                  <a
                    ...
                    aria-label={heroSliderTabs[index]?.line1 || `Xem slide ${index + 1}`}
                  >
                    <img
                      src={slide.image}
            ...
            <div className="mini-banners">
              {subBanners.map((banner) => (
                <a
                  href={banner.href || getCategoryTopicPath(
                    "Khuyến mãi",
                    banner.title || banner.alt || "Khuyến mãi CellphoneS",
                  )}
                  key={banner.id}
...
const tabletQuickLinks = [
  { name: "Máy đọc sách", category: "Máy đọc sách", href: "/tablet/may-doc-sach.html", span: 2 },
  { name: "Kindle", category: "Máy đọc sách", query: "Kindle" },
  { name: "BOOX", category: "Máy đọc sách", query: "BOOX" },
  {
...
const hotTabletModels = [
  { name: "iPad Pro M5", badge: "Hot", span: 2 },
  { name: "iPad Air M4", badge: "Hot", span: 2 },
  { name: "iPad A16", span: 2 },
  { name: "iPad mini 7", span: 2 },
  { name: "Galaxy Tab S11 Series", badge: "Hot", span: 4 },
  { name: "Galaxy Tab S10 Series", span: 3 },
  { name: "Lenovo Legion Tab Gen 5", span: 4 },
  { name: "Xiaomi Poco Pad M1", span: 3 },
  { name: "Huawei MatePad SE", badge: "Mới", span: 3 },
  { name: "Honor Pad X8B", span: 3 },
];
...
const laptopNeedRoutes = {
  "Văn phòng": "/laptop/van-phong.html",
  Gaming: "/laptop/gaming.html",
  "Mỏng nhẹ": "/laptop/mong-nhe.html",
  "Đồ họa - kỹ thuật": "/laptop/do-hoa.html",
  "Sinh viên": "/laptop/sinh-vien.html",
  "Cảm ứng": "/laptop/cam-ung.html",
  "Laptop AI": "/laptop/ai.html",
};

const laptopScreenSizes = [
  { name: "Laptop 13 inch", span: 3 },
  { name: "Laptop 14 inch", span: 3 },
  { name: "Laptop 15.6 inch", span: 3 },
  { name: "Laptop 16 inch", span: 3 },
];
...
const micTypesSquare = [
  ...
  },
  {
    ...
    img: "https://cdn2.cellphones.com.vn/insecure/rs:fill:0:358/q:90/plain/https://cellphones.com.vn/media/catalog/product/m/i/micro-khong-day-paramax-pasion-echo_2_.png",
  },
];
...
const beautyDevices = [
  ...
  },
  {
    ...
    img: "https://cdn2.cellphones.com.vn/insecure/rs:fill:50:50/q:90/plain/https://cellphones.com.vn/media/catalog/product/t/o/tong-do-cat-toc-flyco-fc5908vn-1_1.jpg",
  },
  {
...
const hotTvProducts = [
  {
    name: "Máy lạnh Mijia Pro 1.5 HP Inverter 2025",
    category: "Điều hòa - Máy lạnh",
    badge: "Hot",
    span: 6,
  },
  {
    name: "Tủ lạnh Xiaomi Multidoor 510L 2025",
    category: "Tủ lạnh",
    badge: "Mới",
    span: 6,
  },
  { name: "Tivi di động LG Stanby Me 27inch", category: "Tivi", span: 4 },
  { name: "Giá treo tivi", category: "Tivi", span: 2 },
  {
    name: "Tủ chăm sóc quần áo",
    category: "Tủ chăm sóc quần áo",
    span: 3,
  },
];
