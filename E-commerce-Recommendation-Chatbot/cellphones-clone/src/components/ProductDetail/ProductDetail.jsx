...
const getProductBreadcrumbTrail = (product = {}) => {
  const productNameKey = normalizeOptionKey(product.name);
  const sourceTrail = Array.isArray(product.categoryTrail)
    ? product.categoryTrail.filter((item) => (
      item?.name && normalizeOptionKey(item.name) !== productNameKey
    ))
    : [];
  const breadcrumbContext = normalizeOptionKey([
    product.name,
    product.category,
    ...sourceTrail.map((item) => item.name),
  ].filter(Boolean).join(' '));

  if (/\biphone\s*17\b/.test(breadcrumbContext)) {
    return [
  ...
  const homeIndex = sourceTrail.findIndex((item) => (
    item.id === 'home' ||
    item.href === '/' ||
    normalizeOptionKey(item.name) === 'trang chu'
  ));
  const homeItem = homeIndex >= 0
    ? { ...sourceTrail[homeIndex], id: 'home', name: 'Trang chủ', href: '/' }
    : { id: 'home', name: 'Trang chủ', href: '/' };
  const remainingTrail = sourceTrail.filter((_, index) => index !== homeIndex);

  return [homeItem, ...remainingTrail];
};
...
export default function ProductDetail({ product, currentUser, onGoLogin, onAddToCart, onGoCart }) {
  ...
  const breadcrumbItems = useMemo(() => getProductBreadcrumbTrail(product), [product]);
      ...
      <div className="container">
        <nav className="pdp-breadcrumb" aria-label="Breadcrumb">
          {breadcrumbItems.map((item, index) => (
            <span className="pdp-breadcrumb-node" key={`${item.id}-${index}`}>
              {index > 0 && <span className="pdp-breadcrumb-separator">/</span>}
              <a href={toLocalCellphonesPath(item.href || '/')}>{item.name}</a>
            </span>
          ))}
