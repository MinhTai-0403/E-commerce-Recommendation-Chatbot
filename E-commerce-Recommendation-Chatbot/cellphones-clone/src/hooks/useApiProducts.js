...
const normalizeRequestQuery = (query = {}, limit = 10) => {
  const requestQuery = {
    sort: "latest",
    ...query,
    limit,
  ...
  if (!hasQueryValue(requestQuery.inStock)) {
    delete requestQuery.inStock;
  } else if (requestQuery.inStock === true || requestQuery.inStock === "true") {
    requestQuery.inStock = "true";
  } else if (
    requestQuery.inStock === false ||
    requestQuery.inStock === "false"
  ) {
    requestQuery.inStock = "false";
  }
...
const isGeneratedDescriptionArticle = (product) => {
  ...
  const sections = Array.isArray(product.articleSections)
...
const mergeApiProductWithFallback = (apiProduct, fallbackProduct) => {
  ...
  const keepFallbackDetail = shouldKeepFallbackDetail(
    apiProduct,
    fallbackProduct,
  );

  const fallbackHasArticle = Boolean(
    fallbackProduct.articleHtml || fallbackProduct.articleSections?.length,
  );
  ...
  [
    ...
    const fallbackHasItems = hasItems(fallbackProduct[field]);
    const apiHasItems = hasItems(apiProduct[field]);

    if (fallbackHasItems && (keepFallbackDetail || !apiHasItems)) {
  ...
  if (
    fallbackHasArticle &&
    (keepFallbackDetail || isGeneratedDescriptionArticle(apiProduct))
  ) {
    merged.articleHtml = fallbackProduct.articleHtml || "";
    merged.articleTitle =
      fallbackProduct.articleTitle || apiProduct.articleTitle;
  ...
  ["stockNote", "shortNotice", "statusLabel"].forEach((field) => {
  ...
  if (fallbackProduct.preferLocalDetail) {
    ...
    merged.reviewSummary =
      fallbackProduct.reviewSummary || merged.reviewSummary;

    merged.currentPrice = fallbackProduct.currentPrice || merged.currentPrice;

    merged.originalPrice =
      fallbackProduct.originalPrice || merged.originalPrice;
  }
...
export function useApiProducts(query, fallbackProducts = []) {
  ...
  useEffect(() => {
    ...
    const displayLimit = Number(
      parsedQuery.displayLimit || parsedQuery.limit || 10,
    );
    ...
    const requestQuery = normalizeRequestQuery(parsedQuery, fetchLimit);

    const hasActiveFilter = FILTER_QUERY_KEYS.some((key) =>
      hasQueryValue(parsedQuery[key]),
    );

    const canUseFallback = !hasActiveFilter;
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([product, related]) => {
        ...
        const mergedProduct = mergeApiProductWithFallback(
          apiProduct,
          fallbackProduct,
        );
