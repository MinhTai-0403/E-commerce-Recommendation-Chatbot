...
const detailedFilterGroups = {
  ...
  'screen-size': [
    { title: 'Kích thước màn hình điện thoại', param: 'screenSize', options: ['Dưới 6 inch', '6.1 inch', '6.5 inch', '6.7 inch', 'Trên 6.8 inch'].map((value) => ({ label: value, value })) },
    { title: 'Kích thước màn hình laptop/màn hình', param: 'screenSize', options: ['13 inch', '14 inch', '15.6 inch', '16 inch', '24 inch', '27 inch', '32 inch'].map((value) => ({ label: value, value })) },
  ],
  usage: [
    { title: 'Nhu cầu sử dụng', param: 'usage', options: ['Chơi game', 'Chụp ảnh đẹp', 'Pin trâu', 'Mỏng nhẹ', 'Học tập - văn phòng', 'Đồ họa - thiết kế', 'Livestream - sáng tạo nội dung'].map((value) => ({ label: value, value })) },
  ],
  display: [
...
const getCategoryCriteriaForPage = (page, apiCategory = '') => {
  ...
  if (key.includes('laptop') || key.includes('macbook') || key.includes('pc gaming') || key.includes('man hinh')) return laptopCategoryCriteria;
...
const getDetailedFilterGroups = (item = {}) => (
...
const clearDetailedFilterPath = (page, item = {}) => {
  ...
  return buildCategoryControlPath(page, {
    ...
    facet: item.id === 'all' || item.id === 'price' || item.facet ? '' : page.facet,
            ...
            <div className="category-filter-chips">
              {listingCriteria.map((item) => {
                ...
                const isActive = item.id === 'all'
                  ...
                  : Boolean(detailedValue) || activeFilter === (item.facet || item.filter || item.id);
                ...
                const groups = getDetailedFilterGroups(item);
      ...
      {!loading && products.length > 0 && (
        <div className="info-load-more-wrap">
          ...
          {canLoadMore ? (
            ...
            <small>Đã đạt giới hạn hiển thị {CATEGORY_MAX_LIMIT} sản phẩm. Dùng bộ lọc để thu hẹp kết quả.</small>
          ) : (
